...
const sections: EssaySection[] = [
  {
    heading: { en: 'What the PIN actually gates', et: 'Mida PIN tegelikult valvab' },
    paragraphs: [
      {
        en: 'Smart-ID does not work the way a password manager works. There is no stored secret an agent could copy once and replay forever. The qualified-signing key never exists as a single value anywhere: SK’s terms define the private key as something whose value "is never generated" and which "exists only in the form of its components" (TCU, definitions). One component lives in the app on the subscriber’s phone, protected by the PIN; the signing operation itself runs as a threshold protocol with SK’s server-side signing module.',
        et: 'Smart-ID ei tööta nii, nagu töötab paroolihaldur. Ei ole olemas salvestatud saladust, mille agent saaks korra kopeerida ja lõputult uuesti esitada. Kvalifitseeritud allkirja privaatvõtit ei eksisteeri kusagil ühe tervikliku väärtusena: SK tingimused ütlevad privaatvõtme kohta, et selle väärtust „ei genereerita kunagi“ ja et see „eksisteerib ainult oma komponentide kujul“ (TCU, mõisted). Üks komponent asub rakenduses kasutaja telefonis, kus seda kaitseb PIN; allkirjastamistoiming ise kulgeb jagatud arvutusena koos SK serveripoolse allkirjastamismooduliga.',
      },
      {
      ...
      },
      {
        ...
        et: 'Kaks PIN-koodi, kaks reeglit, üks struktuur: võtme iga kasutamine nõuab kasutamise hetkel uusi, inimese sisestatud aktiveerimisandmeid. Ei ole püsivat avatud olekut, mida agent saaks varastada või jagada.',
      },
    ],
  },
  {
    heading: { en: 'The documents already say no', et: 'Dokumendid ütlevad juba praegu ei' },
    paragraphs: [
      {
      ...
      },
      {
        ...
        et: 'Kasutaja kohustused osutavad samas suunas. Kasutaja peab tagama, et privaatvõtit „kasutatakse tema kontrolli all“ (TCU §5.2.8), peab SK-d teavitama ja sertifikaadid kehtetuks tunnistama, kui võtit võidi kasutada ilma loata (TCU §5.2.11), ning peab need viivitamata kehtetuks tunnistama, kui privaatvõti või PIN-kood „on tema valdusest väljunud“ (TCU §5.2.12). Smart-ID turvajuhised ütlevad kasutajale sama, ainult ilma lepingukeeleta: ära jaga PIN1 ega PIN2 „kellegagi, isegi mitte pereliikmete või sõpradega“ ja ära sisesta kunagi PIN-koodi tehingu jaoks, mida sa ise ei algatanud (Smart-ID KKK, „Kuidas hoida oma nutiseadet ja Smart-ID-d turvaliselt?“).',
      },
      {
        ...
        et: 'See, milline punkt käivitub, sõltub lahenduse ülesehitusest. Agent, kes hoiab PIN-koodi või aktiveerib võtme ilma et kasutaja seda iga tehingu jaoks sisestaks, on kolmas osapool, kes kasutab kasutaja võtit (TCU §3.1.3), ja teeb seda automaatselt (TCU §3.1.4, rQSCD SPS §1.4.2). Agent, kes PIN-koodi ainult oma tarkvara kaudu edasi annab, ei ole automatiseeritud kasutus, kuid PIN on ikkagi läbinud millegi, mida kasutaja ei kontrolli, ja kasutaja kohustus TCU §5.2.12 järgi sertifikaat kehtetuks tunnistada niipea, kui PIN on tema valdusest väljunud, ei oota ära vaidlust selle üle, kuidas see väljus. TCU §5.2.11, mis käsitleb loata kasutamist, kohaldub siis, kui agent tegutseb ilma kasutaja nõusolekuta; kasutaja, kes agendi ise üles seadis, on kasutamise heaks kiitnud, ja just seetõttu langeb risk temale, mitte müüjale.',
      },
    ],
  },
  {
    heading: {
      en: 'What eIDAS adds when a signature is qualified',
      et: 'Mida eIDAS lisab, kui allkiri on kvalifitseeritud',
    },
    paragraphs: [
      {
        en: 'For authentication, the rules above are contractual. For signing, a second layer applies, because Smart-ID’s signing certificate is a qualified certificate (TCU §3, certificate types) and the signature is meant to be a qualified electronic signature (QES).',
        et: 'Autentimise puhul on ülaltoodud reeglid lepingulised. Allkirjastamise puhul lisandub teine kiht, sest Smart-ID allkirjasertifikaat on kvalifitseeritud sertifikaat (TCU §3, sertifikaadi liigid) ja allkiri on mõeldud olema kvalifitseeritud e-allkiri (QES).',
      },
      {
        ...
        et: 'eIDASe määrus (määrus (EL) nr 910/2014, muudetud määrusega (EL) 2024/1183) määratleb täiustatud e-allkirjana muu hulgas allkirja, mis „luuakse e-allkirja andmise andmete abil, mida allkirjastaja saab suure usaldusväärsusega kasutada oma ainukontrolli all“ (artikli 26 punkt c). Kvalifitseeritud e-allkiri on täiustatud e-allkiri, mis on antud kvalifitseeritud e-allkirja andmise vahendiga ja põhineb kvalifitseeritud sertifikaadil (artikkel 3, mõisted), ning artikli 25 lõike 2 järgi on sellel „omakäelise allkirjaga samaväärne õiguslik toime“.',
      },
      {
        en: 'The consequence is structural, not procedural. Sole control is not a property of the person who clicks "I consent" on a settings screen. It is a property of the signing moment: the signature creation data must be under the signatory’s control at the time the signature is created. If an agent activates the signing key, the signature is not created under the signatory’s sole control, so it is not an advanced electronic signature under Article 26, so it cannot be a qualified electronic signature, and the Article 25(2) equivalence does not attach. A signature produced that way may still carry whatever evidential weight national law gives it under Article 25(1), but you have left the regime that makes it the digital equivalent of a handwritten signature.',
        et: 'Tagajärg on struktuurne, mitte menetluslik. Ainukontroll ei ole selle inimese omadus, kes vajutab seadete ekraanil „Nõustun“. See on allkirjastamise hetke omadus: e-allkirja andmise andmed peavad olema allkirjastaja kontrolli all sel hetkel, kui allkiri antakse. Kui võtme aktiveerib agent, ei ole allkiri antud allkirjastaja ainukontrolli all, seega ei ole see täiustatud e-allkiri artikli 26 tähenduses, seega ei saa see olla kvalifitseeritud e-allkiri ja artikli 25 lõike 2 samaväärsus ei rakendu. Nii antud allkiri võib endiselt kanda seda tõenduslikku kaalu, mille riigisisene õigus talle artikli 25 lõike 1 alusel annab, kuid oled väljunud korrast, mis teeb sellest omakäelise allkirja digitaalse vaste.',
      },
      {
        en: 'One nuance before the lawyers arrive: whether sole control under Article 26(c) is judged at the signing moment or as a design-level guarantee of the certified signing device is a live interpretive question, not a settled one. On the design reading, a signature produced through a certified device will in practice be treated as qualified unless sole control is affirmatively challenged, and as of writing no Estonian court has published a ruling on an agent-produced signature as far as the author knows. That does not rescue the PIN handover. Under either reading, handing the activation secret to an agent destroys the per-operation, high-confidence control that the certification was built on, and that is the point that matters here. Validation software never tests who entered the PIN, so an agent-made signature will usually still verify; the fight, when there is one, happens later, and the subscriber carries the burden of explaining why a signature made with their certificate should not stand.',
        et: 'Üks nüanss enne, kui juristid kohale jõuavad: kas artikli 26 punkti c ainukontrolli hinnatakse allkirjastamise hetkel või on see sertifitseeritud allkirjastamisvahendi tasemel antud tagatis, on elav tõlgendusküsimus, mitte lahendatud asi. Kavanditasandi lugemise järgi käsitatakse sertifitseeritud vahendiga antud allkirja praktikas kvalifitseerituna seni, kuni ainukontrolli otsesõnu ei vaidlustata, ja kirjutamise hetkel ei ole autorile teadaolevalt ükski Eesti kohus agendi antud allkirja kohta lahendit avaldanud. See ei päästa PIN-koodi üleandmist. Kummagi lugemise järgi hävitab aktiveerimisandmete agendile andmine selle toimingupõhise ja suure usaldusväärsusega kontrolli, millele sertifitseerimine on üles ehitatud, ja just see on siin oluline. Valideerimistarkvara ei kontrolli kunagi, kes PIN-koodi sisestas, nii et agendi antud allkiri läheb enamasti ikka verifitseerimisest läbi; vaidlus, kui see tuleb, tuleb hiljem, ja kasutaja kanda jääb kohustus selgitada, miks tema sertifikaadiga antud allkiri ei peaks kehtima.',
      },
      {
        en: 'One caveat on scope: eIDAS sole control governs signatures, not sessions. PIN1 authentication is not a signature, and nothing in eIDAS stops a user from granting an agent a token that their own authentication produced. What the contract stops is the agent performing the authentication itself: TCU §3.1.3 prohibits enabling another party to use the private key, and that clause covers both certificates, not just the signing one. Delegating the result of authentication and delegating the act of authenticating are different operations, and the pattern recommended below depends on that distinction.',
        et: 'Üks täpsustus ulatuse kohta: eIDASe ainukontroll puudutab allkirju, mitte seansse. PIN1-ga autentimine ei ole allkiri ja miski eIDASes ei keela kasutajal anda agendile juurdepääsutõendit, mille tema enda autentimine tekitas. Leping keelab selle, et agent teeks autentimise ise: TCU §3.1.3 keelab võimaldada teisel osapoolel privaatvõtit kasutada, ja see punkt katab mõlemat sertifikaati, mitte ainult allkirjasertifikaati. Autentimise tulemuse delegeerimine ja autentimise toimingu delegeerimine on kaks eri asja ning allpool soovitatav muster tugineb just sellele vahele.',
      },
    ],
  },
  {
    heading: {
      en: 'Why "delegated credentials" do not change this',
      et: 'Miks „delegeeritud mandaadid“ seda ei muuda',
    },
    paragraphs: [
      {
        en: 'The counter-argument is usually some version of "we’ll issue the agent a delegated credential". The phrase sounds technical and modern, which makes it a convenient wrapper for a PIN handover. It is worth being precise about what it can and cannot mean.',
        et: 'Vastuargument on tavaliselt mingi variant sellest, et „me väljastame agendile delegeeritud mandaadi“. Fraas kõlab tehniliselt ja moodsalt, mis teeb sellest mugava ümbrise PIN-koodi üleandmisele. Tasub olla täpne selles, mida see saab ja mida ei saa tähendada.',
      },
      {
      ...
      },
      {
        ...
        et: 'Delegeeritud mandaat, mille agent saab ilma inimeseta aktiveerida, ei ole delegeerimine üheski tähenduses, mida määrus tunneb. See on mandaadi üleandmine ja ülaltoodud dokumendid käsitlevad mandaadi üleandmist kehtetuks tunnistamise sündmusena, mitte kavandivalikuna. Delegeeritud mandaat, mis nõuab allkirjastamise hetkel ikkagi inimeselt PIN-koodi sammu, on tõeline delegeerimine, kuid see ei ole uus tehnoloogia: see on tavaline nõusolekuvoog, mille ümber on ehitatud lisakihte, ja PIN-koodi samm on täpselt see osa, mida agent üle võtta ei saa.',
      },
      {
        ...
        et: 'Siin on ka nimetusest tulenev lõks. TLS-is tähendavad „delegeeritud mandaadid“ (RFC 9345) hoopis midagi muud: lühiajalist võtit, mida server esitab oma sertifikaadi omaniku nimel, et sertifikaat ise igapäevasest kasutusest välja jääks. See on serveripoolne TLS-mehhanism, millel ei ole midagi öelda inimese allkirja PIN-koodi kohta. Termin teeb neis vestlustes liiga palju tööd; esimene küsimus, kui keegi pakub agendi allkirjastamiseks „delegeeritud mandaate“, on see, millist protokolli ta silmas peab.',
      },
    ],
  },
  {
    heading: {
      en: 'The pattern that works: delegate authority, keep the credential',
      et: 'Muster, mis töötab: delegeeri volitus, hoia mandaat enda käes',
    },
    paragraphs: [
      {
      ...
      },
      {
        ...
        et: 'Agent saab volituse. Kasutaja hoiab mandaadi enda käes. Iga selles raportis tsiteeritud dokument sobib kokku esimese ja kolmanda sammuga; kogu nende raskus langeb sellele, kes püüab hüpata maailma, kus kolmas samm toimub ilma inimeseta.',
      },
    ],
  },
  {
    heading: { en: 'What this does not fix', et: 'Mida see ei lahenda' },
    paragraphs: [
      {
        en: 'Reading the documents is not the same as holding the line, and three limits deserve to be stated plainly.',
        et: 'Dokumentide lugemine ei ole sama, mis piiri hoidmine, ja kolm piirangut väärivad otse välja ütlemist.',
      },
      {
        en: "First, this is a document-level analysis. A contract clause does not enforce itself: the mechanisms that actually stop an agent from using a leaked PIN are the same ones that stop a fraudster, and the site’s prior Smart-ID research shows how the approval layer can fail socially even when the protocol holds. An attacker who can talk a user into approving a transaction does not need to automate the PIN at all. Social engineering is a separate problem, and that earlier research is the better reference for it.",
        et: 'Esiteks on see dokumenditasandi analüüs. Lepingupunkt ei jõusta end ise: mehhanismid, mis päriselt takistavad agendil lekkinud PIN-koodi kasutamast, on samad, mis takistavad petturit, ja saidi varasem Smart-ID uuring näitab, kuidas kinnituskiht võib sotsiaalselt puruneda ka siis, kui protokoll peab. Ründaja, kes suudab kasutaja tehingut kinnitama rääkida, ei pea PIN-koodi üldse automatiseerima. Sotsiaalne manipuleerimine on eraldi probleem ja see varasem uuring on selle jaoks parem viide.',
      },
      {
        en: 'Second, "compliant" and "enforced" are different words. A bank or service provider may tolerate automation in practice, particularly for low-risk PIN1 flows, and SK’s own documents are what SK publishes rather than what every relying party audits. Banks also layer PSD2 strong customer authentication rules on top for payment contexts, which this report does not analyse. But tolerance is not authorisation, and the asymmetry matters: the subscriber carries the revocation duty under TCU §5.2.12 if a PIN leaves their possession, so the person who loses in a dispute is the user, not the agent vendor.',
        et: 'Teiseks on „nõuetele vastav“ ja „jõustatud“ kaks eri sõna. Pank või teenusepakkuja võib automatiseerimist praktikas taluda, eriti madala riskiga PIN1-voogudes, ja SK enda dokumendid on see, mida SK avaldab, mitte see, mida iga usaldav osapool auditeerib. Pangad lisavad makseolukordades peale ka PSD2 tugeva kliendi autentimise reeglid, mida see raport ei analüüsi. Kuid talumine ei ole luba ja asümmeetria loeb: kasutaja kanda jääb TCU §5.2.12 järgne kehtetuks tunnistamise kohustus, kui PIN tema valdusest väljub, nii et vaidluses kaotab kasutaja, mitte agendi müüja.',
      },
      {
        en: 'Third, none of this is legal advice, and SK’s terms are amended from time to time. Anyone building an agent against Smart-ID should check the current terms with SK or their relying party rather than take this reading as the final word. The reading here reflects the documents as published on 8 September 2026.',
        et: 'Kolmandaks ei ole miski siin õigusnõu ja SK tingimusi muudetakse aeg-ajalt. Igaüks, kes ehitab agenti Smart-ID vastu, peaks kehtivad tingimused SK-lt või oma usaldavalt osapoolelt üle küsima, mitte võtma seda lugemist lõpliku sõnana. Siinne lugemine kajastab dokumente sellisena, nagu need olid avaldatud 8. septembril 2026.',
      },
    ],
  },
];
...
const sources: Source[] = [
  {
    label: {
      en: 'SK ID Solutions AS, "Terms and Conditions for Use of Certificates of Qualified Smart-ID"',
      et: 'SK ID Solutions AS, "Terms and Conditions for Use of Certificates of Qualified Smart-ID"',
    },
    url: 'https://www.skidsolutions.eu/wp-content/uploads/2026/03/SK-TCU-EID-QUALIFIED-SMART-ID-EN-20260501.pdf',
    note: {
      en: 'valid from 1 May 2026 (SK-TCU-EID-QUALIFIED-SMART-ID-EN)',
      et: 'kehtib alates 1. maist 2026 (SK-TCU-EID-QUALIFIED-SMART-ID-EN)',
    },
  },
  {
    label: {
      en: 'SK ID Solutions AS, "SK-rQSCD Management of Remote Qualified Electronic Signature Creation Device Service Practice Statement"',
      et: 'SK ID Solutions AS, "SK-rQSCD Management of Remote Qualified Electronic Signature Creation Device Service Practice Statement"',
    },
    ...
    note: {
      en: 'version 1.0, effective 1 May 2026',
      et: 'versioon 1.0, kehtib alates 1. maist 2026',
    },
  },
  {
    label: {
      en: 'Smart-ID FAQ, "PIN codes: what are they?"',
      et: 'Smart-ID FAQ, "PIN codes: what are they?"',
    },
    url: 'https://www.smart-id.com/help/faq/pin-codes/pin-codes-what-are-they/',
  },
  {
    label: {
      en: 'Smart-ID FAQ, "How to keep your smart device and Smart-ID safe?"',
      et: 'Smart-ID FAQ, "How to keep your smart device and Smart-ID safe?"',
    },
    url: 'https://www.smart-id.com/help/faq/security/how-to-keep-your-smart-device-and-smart-id-safe/',
  },
  {
    label: {
      en: 'Regulation (EU) No 910/2014 (eIDAS), in particular Articles 3, 25 and 26, as amended by Regulation (EU) 2024/1183',
      et: 'Määrus (EL) nr 910/2014 (eIDAS), eelkõige artiklid 3, 25 ja 26, muudetud määrusega (EL) 2024/1183',
    },
    url: 'https://eur-lex.europa.eu/legal-content/EN/TXT/?uri=CELEX:32014R0910',
  },
  {
    label: {
      en: 'RFC 6749, "The OAuth 2.0 Authorization Framework"',
      et: 'RFC 6749, "The OAuth 2.0 Authorization Framework"',
    },
    url: 'https://www.rfc-editor.org/rfc/rfc6749',
  },
  {
    label: {
      en: 'RFC 8693, "OAuth 2.0 Token Exchange"',
...
const disclosureParagraphs: Bi[] = [
  ...
  {
    en: 'The author has published prior Smart-ID protocol research, disclosed to the vendor before publication, and works professionally on identity and anti-fraud systems; that is the context in which this reading should be judged. Research conduct follows the site’s ',
    et: 'Autor on varem avaldanud Smart-ID protokolliuuringuid, mis avalikustati müüjale enne avaldamist, ja töötab ametialaselt identiteedi- ja pettusevastaste süsteemidega; see on kontekst, milles seda lugemist tuleks hinnata. Uurimistöös järgib autor saidi ',
  },
];
...
const thesis: Bi = {
  ...
  et: 'PIN on allkirja aktiveerimise andmed: iga toimingu jaoks uus, kunagi salvestamata ja kohustuslikult kasutaja ainukontrolli all. Skeem, mis annab selle agendile, on mandaadi üleandmine, mitte delegeerimine; nõuetele vastav muster on piiratud ulatusega tõendid pluss inimese tehtud PIN-koodi samm allkirjastamise hetkel.',
};
