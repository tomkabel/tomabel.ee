...
const openingParagraphs: Bi[] = [
  {
    en: 'Read the five fixes back to back and a pattern shows up. Make the verification code mandatory: a regulator has to require it. Make Smart-ID+ the default, same-device where possible: SK and the bank both have to ship and roll it out. Bind the approval to the transaction: SK has to change what the app shows. Shift the liability: a legislature or a court has to move it. Use the regulatory floor: NIS2 and eIDAS 2.0 have to be enforced. Five fixes, five different actors who all have to move before anything changes for the person holding the phone.',
    et: 'Loe need viis parandust järjest läbi ja muster ilmneb. Tee kinnituskoodi kontroll kohustuslikuks: regulaator peab seda nõudma. Tee Smart-ID+ vaikimisi valikuks, samal seadmel kus võimalik: nii SK kui pank peavad selle välja andma ja kasutusele võtma. Seo kinnitus tehinguga: SK peab muutma, mida rakendus kasutajale näitab. Nihuta vastutus: seadusandja või kohus peab selle ümber tõstma. Kasuta regulatiivset alusmiinimumi: NIS2 ja eIDAS 2.0 peavad olema jõustatud. Viis parandust, viis erinevat osalist, kes kõik peavad liikuma, enne kui midagi muutub inimese jaoks, kelle käes on telefon.',
  },
  {
    en: "The report already names why that wait is not a short one. SK ID Solutions is owned by Swedbank and SEB, the banks that would have to absorb the cost of hardening it. Waiting for that structure to volunteer a fix is not a plan, it is a bet against the owner's own incentives.",
    et: 'Raport ise nimetab, miks see ootamine ei ole lühike. SK ID Solutions kuulub Swedbankile ja SEB-le, pankadele, kes peaksid kandma selle karastamise kulu. Oodata, et see omandistruktuur vabatahtlikult paranduse pakuks, ei ole plaan, vaid kihlvedu omaniku enda huvide vastu.',
  },
];

const sections: EssaySection[] = [
  {
    heading: { en: 'What the sixth fix notices', et: 'Mida kuues parandus märkab' },
    paragraphs: [
      {
      ...
      {
        en: 'That gap is visible without touching the Smart-ID protocol at all, because it is not a property of the signature, it is a property of the session the bank is already terminating. Every session a bank\'s edge accepts carries network-layer characteristics the client cannot fully choose: TLS handshake shape, HTTP/2 or HTTP/3 frame timing, connection-level metadata. A bank that fingerprints those characteristics when it issues a QR code, and again when the PIN2 approval for the transaction it authorized comes back, can ask a narrower question than any of the report\'s five fixes ask: is the session confirming this transaction the same session I showed the transaction to? A relay, by construction, answers no.',
        et: 'See lõhe on nähtav ilma Smart-ID protokolli puudutamatagi, sest see ei ole allkirja omadus, vaid selle seansi omadus, mida pank juba lõpetab. Iga seanss, mille pank oma serva juures vastu võtab, kannab võrgutaseme tunnuseid, mida klient täielikult valida ei saa: TLS-käepigistuse kuju, HTTP/2 või HTTP/3 kaadrite ajastus, ühendustaseme metaandmed. Pank, mis jäädvustab need tunnused QR-koodi väljastamise hetkel ja uuesti hetkel, mil tuleb tagasi PIN2-kinnitus tehingu jaoks, mille ta autoriseeris, saab esitada kitsama küsimuse kui ükski raporti viiest parandusest esitab: kas seanss, mis seda tehingut kinnitab, on sama seanss, millele ma tehingut näitasin? Relay vastab konstruktsiooni järgi eitavalt.',
      },
      {
        en: "This is the sixth fix. It does not need SK to change the ACSP_V2 message, does not need a bank to change what its app shows the user, and does not need a regulator to write anything down. It needs a bank's own backend to keep a fingerprint of a session it already holds and compare it to itself a few seconds later.",
        et: 'See ongi kuues parandus. See ei vaja, et SK muudaks ACSP_V2 sõnumit, ei vaja, et pank muudaks, mida tema rakendus kasutajale näitab, ega vaja, et regulaator midagi kirja paneks. Vaja on, et panga enda taustasüsteem hoiaks seansi sõrmejälge, mis tal juba on, ja võrdleks seda mõne sekundi pärast iseendaga.',
      },
    ],
  },
  {
  ...
  {
    heading: { en: 'Who can ship it, and what it is worth', et: 'Kes saab selle tarnida ja mida see väärt on' },
    paragraphs: [
      {
        en: "The honest case for the sixth fix is narrow and specific: a bank's own fraud or security team can build it, on infrastructure that bank already operates, without a vendor's roadmap or a legislature's calendar standing between the decision and the deployment.",
        et: 'Aus argument kuuenda paranduse kasuks on kitsas ja konkreetne: panga enda pettuse- või turvameeskond saab selle ehitada taristule, mida see pank juba käitab, ilma et müüja tegevuskava või seadusandja kalender seisaks otsuse ja juurutamise vahel.',
      },
      {
        en: "The honest limit is the same one this site makes about every client-side or network-side signal: it is a gate, not a verdict. A session-continuity mismatch is a reason to add friction, ask for a second confirmation, or hold a transfer for review. It is not proof of fraud on its own, and a system that treats it as a verdict will eventually lock out a legitimate user whose network path changed for an ordinary reason. Use it the way the report says to use everything else here: as one signal among several, never as the whole decision.",
        et: 'Aus piirang on sama, mida see sait ütleb iga kliendipoolse või võrgupoolse signaali kohta: see on värav, mitte kohtuotsus. Seansi järjepidevuse mittevastavus on põhjus lisada hõõrdumist, küsida teist kinnitust või hoida ülekanne läbivaatuseks kinni. See ei ole iseenesest pettuse tõend, ja süsteem, mis kohtleb seda kohtuotsusena, lukustab lõpuks välja seadusliku kasutaja, kelle võrgutee muutus tavalisel põhjusel. Kasuta seda samamoodi, nagu raport ütleb kasutada kõike muud siin: ühe signaalina paljude seas, mitte kunagi kogu otsusena.',
      },
      {
        en: 'One disclosed interest, stated plainly: session-continuity scoring at the network edge is the product category I build in professionally. That is a reason to read this essay with your own skepticism intact, not a reason to distrust the argument, since the argument does not depend on any specific vendor, mine included, being the one a bank chooses.',
        et: 'Üks avalikustatud huvi, otse öelduna: seansi järjepidevuse hindamine võrgu servas on tootekategooria, mida ma ehitan ametialaselt. See on põhjus lugeda seda esseed oma skepsis alles hoides, mitte põhjus argumenti umbusaldada, sest argument ei sõltu sellest, et konkreetne müüja, minu oma kaasa arvatud, oleks see, kelle pank valib.',
      },
    ],
  },
];

export default function TheFixThatDoesntNeedSkPage() {
  ...
  return (
    <article>
          ...
          <section className="mt-16 max-w-3xl">
            ...
            <div className="mt-6 space-y-4 text-lg leading-relaxed text-muted">
              <p>
                {isEn ? (
                  <>
                    The report this sixth fix answers is{' '}
                    ...
                    , which documents the signing relay in full and lists the other five. The disclosed research corpus behind both,{' '}
                    ...
                    , carries the more technical version of this argument for readers who want the protocol-level detail this essay leaves out.
                  </>
                ) : (
                  <>
                    Raport, millele see kuues parandus vastab, on{' '}
                    <Link to="/disclosures/smart-id-achilles-heel" className="text-accent underline decoration-border underline-offset-4 hover:decoration-accent">
                    ...
                    , kannab selle argumendi tehnilisemat versiooni lugejatele, kes tahavad protokollitaset detaili, mille see essee vahele jätab.
                  </>
                )}
              </p>
            </div>
      ...
      <ArticleProof
        ...
        expectedSha256="6afecdcfdbbbafe513cc5ead28539b32596b31a5f760e35beb0a66684186a34f"
      />
    </article>
  );
}
